...
bunnypos = ti.Vector.field(3,float,shape=(num_particles,))
bunnypos.from_numpy(bunny)
...
def init():
    for i in particles.p:
        particles.p[i] = ti.Vector([bunnypos[i].x, bunnypos[i].y, bunnypos[i].z])
        particles.vort[i] = ti.Vector([0.0, 0.0, 0.0])
        particle_id[i] = i
