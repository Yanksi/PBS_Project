...
def pbf_solve():
    ...
    for p in particles.p:
        if( (particles.p[p]-center).norm() < 12.0):
            u = (particles.p[p] - center).normalized()*12.0
            particles.dp[p] += (u-particles.p[p] + center)
    plane =  ti.Vector([1.0, 1.0, 1.0, -15.0])
    for p in particles.p:
        if( particles.p[p].dot(plane[0:3])+plane[3] < 0.0):
            u = - ( ti.math.dot(particles.p[p],plane[0:3])+plane[3] )/plane[0:3].norm()
            particles.dp[p] += u*plane[0:3]
