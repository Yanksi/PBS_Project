...
@ti.func
def boundary_condition(v,lowx):
    v[0]=ti.math.clamp(v[0],lowx,world[0] - padding)
    v[1]=ti.math.clamp(v[1],.5,world[1] - padding)
    v[2]=ti.math.clamp(v[2],.5,world[2] - padding)
    return v
...
@ti.kernel
def pbf_pred_pos(ad: float, ws: float, lowx: float):
    ...
    for i in particles:
        ...
        # ---predict position---
        particles.p[i] = boundary_condition(particles.p[i]+ dt * particles.v[i],lowx)
...
def main():
    ...
    while window.running:
        ...
        if window.is_pressed(ti.ui.LEFT): lowx = max(0, lowx-0.2)
        if window.is_pressed(ti.ui.RIGHT): lowx = min(10., lowx+0.2)
